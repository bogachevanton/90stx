jackpot-tickets-90stx-april-22
...
;; Define NFT token
(define-non-fungible-token jackpot-ticket-90stx-april-22 uint)
...
(define-data-var mint-limit uint u2250)
(define-data-var mintpass-sale-active bool false)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 80) "ipfs://QmeF7hp7XhypUEUwwCHtQztEHnCyhJcFh3AfJuBS722Ym3/")
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? jackpot-ticket-90stx-april-22 id)))
...
;; Get the token URI
(define-read-only (get-token-uri (id (string-ascii 4)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? jackpot-ticket-90stx-april-22 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
  (let 
    ...
    (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? jackpot-ticket-90stx-april-22 id owner))
    (map-set token-count
      owner
      (- owner-balance u1))
    (ok true)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jackpot-ticket-90stx-april-22 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jackpot-ticket-90stx-april-22 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jackpot-ticket-90stx-april-22 id) ERR-NOT-FOUND))
...
(define-public (pick-win-jacpot-ticket)
