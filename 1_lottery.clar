...
;; Define NFT token
(define-non-fungible-token lottery-ticket-90stx-draw-1 uint)
...
(define-constant WALLET_1 'principal)
(define-constant WALLET_2 'principal)
...
(define-data-var base-uri (string-ascii 80) "ipfs://QmTf2DT8YMRjuDRmjkf5ceVU5UgdnB25SML9XrzQDjsR3v/")
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? lottery-ticket-90stx-draw-1 id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? lottery-ticket-90stx-draw-1 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
  (let 
    ...
    (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? lottery-ticket-90stx-draw-1 id owner))
    (map-set token-count
      owner
      (- owner-balance u1))
    (ok true)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? lottery-ticket-90stx-draw-1 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? lottery-ticket-90stx-draw-1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? lottery-ticket-90stx-draw-1 id) ERR-NOT-FOUND))
