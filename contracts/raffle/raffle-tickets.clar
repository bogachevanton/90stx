;; tickets-90stx-raffle-N, where N - raffle number like 1,2,3...
...
;; Define NFT token
(define-non-fungible-token ticket-90stx-raffle-N uint) ;; where N - raffle number like 1,2,3...
...
(define-constant WALLET_1 'principal) ;; Fund wallet
(define-constant WALLET_2 'principal) ;; Team wallet
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? ticket-90stx-raffle-N id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? ticket-90stx-raffle-N next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
  (let 
    ...
    (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? ticket-90stx-raffle-N id owner))
    (map-set token-count
      owner
      (- owner-balance u1))
    (ok true)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ticket-90stx-raffle-N id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ticket-90stx-raffle-N id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ticket-90stx-raffle-N id) ERR-NOT-FOUND))
