...
;; Define NFT token
(define-non-fungible-token ticket-90stx-raffle-3 uint)
...
(define-data-var base-uri (string-ascii 80) "ipfs://QmR1X5gfcNANiUyt69aVMLcxfg4MEDKXeLFERH9otxLg5o/")
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? ticket-90stx-raffle-3 id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? ticket-90stx-raffle-3 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
  (let 
    ...
    (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? ticket-90stx-raffle-3 id owner))
    (map-set token-count
      owner
      (- owner-balance u1))
    (ok true)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ticket-90stx-raffle-3 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ticket-90stx-raffle-3 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ticket-90stx-raffle-3 id) ERR-NOT-FOUND))
