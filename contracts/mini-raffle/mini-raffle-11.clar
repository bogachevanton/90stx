...
;; Define NFT token
(define-non-fungible-token ticket-90stx-mini-raffle-11 uint)
...
(define-data-var base-uri (string-ascii 80) "ipfs://QmRFujqdmCdpk9M3v6MzBPi9wWwQfJFVKZ5ecg8MEBGT5j/")
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? ticket-90stx-mini-raffle-11 id)))
...
(define-private (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? ticket-90stx-mini-raffle-11 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint))
  (let
    ((owner (unwrap! (nft-get-owner? ticket-90stx-mini-raffle-11 id) ERR-NOT-FOUND))
    (owner-balance (get-balance owner)))
    (asserts! (is-sender-owner id) ERR-NOT-AUTHORIZED)
    (begin
        (try! (nft-burn? ticket-90stx-mini-raffle-11 id owner))
        (map-set token-count
        owner
        (- owner-balance u1))
    (ok true))))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ticket-90stx-mini-raffle-11 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ticket-90stx-mini-raffle-11 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ticket-90stx-mini-raffle-11 id) ERR-NOT-FOUND))
