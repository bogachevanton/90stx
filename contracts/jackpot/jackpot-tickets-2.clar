...
;; Define NFT token
(define-non-fungible-token jackpot-ticket-90stx-may-22-1 uint)
...
;; Variables
(define-data-var last-id uint u192)
(define-data-var mint-limit uint u2250)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 80) "ipfs://QmUswLuSKaEmHeX1Ff3pomPQDMxtQV3BJ891yc523HfzMa/")
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? jackpot-ticket-90stx-may-22-1 id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? jackpot-ticket-90stx-may-22-1 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
  (let 
    ...
    (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? jackpot-ticket-90stx-may-22-1 id owner))
    (map-set token-count
      owner
      (- owner-balance u1))
    (ok true)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jackpot-ticket-90stx-may-22-1 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jackpot-ticket-90stx-may-22-1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jackpot-ticket-90stx-may-22-1 id) ERR-NOT-FOUND))
