jackpot-tickets-90stx-M-Y-N, where M is month (may, june, july...), Y is year 22,23,... and N is number like 1,2,3... (serial number within a month)

;; Traits
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait) ;; SIP-009 trait
(use-trait commission-trait .commission-trait.commission)

;; Define NFT token
(define-non-fungible-token jackpot-ticket-90stx-M-Y-N uint) ;; where M is month (may, june, july...), Y is year 22,23,... and N is number like 1,2,3... (serial number within a month)
...
(define-data-var mint-limit uint u0) ;; place mint limit
(define-data-var mintpass-sale-active bool false)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 80) "ipfs://CID/") ;; place metadata CID
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? jackpot-ticket-90stx-M-Y-N id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? jackpot-ticket-90stx-M-Y-N next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
  (let 
    ...
    (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? jackpot-ticket-90stx-M-Y-N id owner))
    (map-set token-count
      owner
      (- owner-balance u1))
    (ok true)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jackpot-ticket-90stx-M-Y-N id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jackpot-ticket-90stx-M-Y-N id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jackpot-ticket-90stx-M-Y-N id) ERR-NOT-FOUND))
...
;; Pick winner in jackpot
(define-constant REACHED-BLOCK-PICK-LIMIT (err u701))
...
(define-public (pick-win-jackpot-ticket)
...
(define-public (set-jackpot-limit (limit uint))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (asserts! (< (var-get limit-tickets) limit) ERR-MINT-LIMIT)
...
(define-read-only (get-jackpot-limit)
