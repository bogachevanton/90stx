...
(define-public (claim-three)
  (begin
    ...
    (try! (claim))
    (try! (claim))
    (try! (contract-call? .jackpot-tickets-90stx-june-22-1 mint tx-sender))
    (ok true)))
...
(define-public (mint-jackpot-ticket (new-owner principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (try! (contract-call? .jackpot-tickets-90stx-june-22-1 mint new-owner))
    (ok true)))
...
(define-private (mint-ticket (new-owner principal))
  (begin
    ...
    (try! (contract-call? .tickets-90stx-raffle-3 mint new-owner))
    (try! (contract-call? .jackpot-tickets-90stx-june-22-1 mint new-owner))
    (ok true)))
...
(define-private (burn-ticket (id uint))
  (let
    ((owner (unwrap! (unwrap! (contract-call? .tickets-90stx-raffle-3 get-owner id) ERR-FATAL) ERR-NOT-FOUND)))
    (asserts! (is-eq owner tx-sender) ERR-NOT-AUTHORIZED)
    (asserts! (var-get burn-active) ERR-BURN-NOT-ACTIVE)
    (try! (contract-call? .tickets-90stx-raffle-3 burn id tx-sender))
    (ok true)))

;; Internal - Burn jackpot ticket
(define-private (burn-jackpot (id uint))
  (let
    ((owner (unwrap! (unwrap! (contract-call? .jackpot-tickets-90stx-june-22-1 get-owner id) ERR-FATAL) ERR-NOT-FOUND)))
    (asserts! (is-eq owner tx-sender) ERR-NOT-AUTHORIZED)
    (asserts! (var-get burn-active) ERR-BURN-NOT-ACTIVE)
    (try! (contract-call? .jackpot-tickets-90stx-june-22-1 burn id tx-sender))
    (ok true)))

;; Register this contract as allowed to mint
(as-contract (contract-call? .tickets-90stx-raffle-3 set-mint-address))
(as-contract (contract-call? .jackpot-tickets-90stx-june-22-1 set-mint-address))
