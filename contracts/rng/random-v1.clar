;; RNG v1 90 STX Raffle 2
;; 90STX.XYZ PLATFORM

(define-constant CONTRACT-OWNER tx-sender)
(define-constant ERR-NOT-AUTHORIZED (err u101))
(define-constant REACHED-BLOCK-PICK-LIMIT (err u102))
...
(define-private (pick-win-ticket)
    (let ((byte-idx (var-get b-idx)))
            (begin
                (set-vrf)
                (asserts! (< byte-idx u64) REACHED-BLOCK-PICK-LIMIT)
                (let ((picked-idx (mod (rand byte-idx) (var-get limit-tickets)))
                    ...
                    (if (< byte-idx u62) (var-set b-idx (+ (var-get b-idx) u2)) (var-set b-idx u0))
