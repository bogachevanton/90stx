...
;; Claim NFT (1 lottery + 1 jackpot)
(define-public (claim)
  (if (var-get mintpass-sale-active)
    (mintpass-mint tx-sender)
    (public-mint tx-sender)))

;; Claim 3 NFT (3 lottery + 4 jackpot)
(define-public (claim-three)
  (begin
    (try! (claim))
    (try! (claim))
    (try! (claim))
    (try! (contract-call? .jackpot-tickets-90stx-april-22 mint tx-sender))
    (ok true)))

;; Claim 5 NFT (5 lottery + 7 jackpot)
...
;; Claim 10 NFT (10 lottery + 15 jackpot)
...
;; Claim 1 jackpot (only contract owner)
(define-public (mint-jackpot-ticket (new-owner principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (try! (contract-call? .jackpot-tickets-90stx-april-22 mint new-owner))
    (ok true)))

;; Check lottery ticket
(define-public (check-lot-ticket (id uint))
  (begin
    (try! (burn-lot id))
    (ok true)))
...
(define-private (mintpass-mint (new-owner principal))
  (let ((presale-balance (get-presale-balance new-owner)))
    ...
    (map-set presale-count
              new-owner
              (- presale-balance u1))
  (try! (contract-call? .lottery-tickets-90stx-draw-1 mint new-owner))
  (try! (contract-call? .jackpot-tickets-90stx-april-22 mint new-owner))
  (ok true)))
...
;; Internal - Burn lottery ticket
(define-private (burn-lot (id uint))
  (let
    ((owner (unwrap! (unwrap! (contract-call? .lottery-tickets-90stx-draw-1 get-owner id) ERR-FATAL) ERR-NOT-FOUND)))
    (asserts! (is-eq owner tx-sender) ERR-NOT-AUTHORIZED)
    (try! (contract-call? .lottery-tickets-90stx-draw-1 burn id tx-sender))
    (ok true)))

;; Internal - Burn jackpot ticket
(define-private (burn-jackpot (id uint))
  (let
    ((owner (unwrap! (unwrap! (contract-call? .jackpot-tickets-90stx-april-22 get-owner id) ERR-FATAL) ERR-NOT-FOUND)))
    (asserts! (is-eq owner tx-sender) ERR-NOT-AUTHORIZED)
    (try! (contract-call? .jackpot-tickets-90stx-april-22 burn id tx-sender))
    (ok true)))

;; Register this contract as allowed to mint
(as-contract (contract-call? .lottery-tickets-90stx-draw-1 set-mint-address))
(as-contract (contract-call? .jackpot-tickets-90stx-april-22 set-mint-address))

;; Mintpasses
(map-set presale-count 'principal u1)